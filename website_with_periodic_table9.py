...
def load_composition_database() -> pd.DataFrame:
    try:
        return pd.read_parquet("7_streamlit_database.parquet")
    except:
        df = pd.read_excel("7_streamlit_database.xlsx")
        df.to_parquet("7_streamlit_database.parquet")
    return df
    ...
    if weighting_method == "Entropy Weighting":
        ...
        with preset_cols[2]:
            if st.button("Short-term goal", key="preset_short_term"):
                st.session_state.preset_weights = {
                    col: 5 if col in ("Production (ton)", "HHI (USGS)", "CO2 footprint average (kg/kg)",
                                      "Water usage average (l/kg)", "Embodied energy average (MJ/kg)") else 1
                    for col in available_criteria
                }
                st.rerun()
